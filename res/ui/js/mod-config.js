...
// 夜航手册配置队列。每一项结构：{ grade, num, boci, level }。
var modConfigList = [];
...
function renderModList() {
    ...
    if (modConfigList.length === 0) {
        html = '<tr><td colspan="5" style="text-align:center; color:#999; font-size:12px;">暂无计划，请上方添加</td></tr>';
    } else {
        $.each(modConfigList, function(index, item) {
            html += '<tr>';
            html += '  <td>' + item.grade + '级</td>';
            html += '  <td>' + item.num + '次</td>';
            html += '  <td>' + item.boci + '轮</td>';
...
function bindModConfigEvents(layer) {
    $("#btn_add_mod_config").click(function() {
        ...
        if (!bociVal || bociVal <= 0) {
            layer.msg("轮次必须大于0", {icon: 5, anim: 6});
            return;
        }
