    ...
    def level_qiyu(self):
        ...
        if res:
            ...
            for i in range(50):
                ...
                if map_type == 0:
                    # 战斗奇遇
                    res = self.is_text_re_in_ocr(rect=[773,459,1252,703],pattern="(附和|是的|沉默|加入他们|回应)")
                    if res:
                        print("选择不战斗")
                        x = res[0].x
                        y = res[0].y
            ...
            if res:
                ...
                self.sleep(1)
                self.click_color_to_color(mijin_color,"退出炮台确定",mijin_color,"炮台结算界面",x=776,y=413)
                self.sleep(1)
                for i in range(50):
                    r1 = self.find_my_color(common_color,"下蹲按钮")
                    ...
                    if r1 and (not r2):
                        return True
                    self.click(634,659,after_sleep=0.1)
                return False
            else:
                # 判断是否有门，防止游戏bug导致卡死
    ...
    def go_to_shop(self):
        ...
        for i in range(10):
            if self.find_my_color(common_color,"左上角红色退出"):
                ...
                break
        # self.click_color_to_color(common_color,"左上角红色退出",common_color,"下蹲按钮",x=44,y=31)
        self.sleep(1)
