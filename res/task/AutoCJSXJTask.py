...
class AutoCJSXJTask(BaseTask):
    ...
    def set_skill_config(self):
        # 初始化技能配置
        self.cjsxj_role_skill = self.uiconfig.get('cjsxj_role_skill', '0')
    ...
    def init_task(self):
        ...
        # 自定义参数在任务初始化阶段一次性注入；进入每个新副本时只 reset 计时器。
        self.cjsxj_combat.configure(self.cjsxj_role_skill, skill_config)
        self.isfail = False
    ...
    def go_to_level(self):
        ...
        for i in range(3):
            self.click(758,617)
        self.sleep(2)
        self.click_until_ocr(x=1107, y=513, rect=[87,8,221,55], pattern="戏剧")
        self.sleep(1)
        print("成功进入沉浸式戏剧")
        return True
    ...
    def go_in_level(self):
        ...
        if res:
            print("未完成的关卡星星")
            self.click_until_ocr(x=res.x,y=res.y, rect=[1093,606,1263,717], pattern="前往")
    ...
    def quit_level(self):
        ...
        self.sleep(1)
        self.click_until_ocr(x=1131, y=681, rect=[1093,606,1263,717], pattern="前往", time_out=20)
        self.sleep(1)
        self.click_until_ocr(x=44, y=32, rect=[87, 8, 221, 55], pattern="戏剧", time_out=20)
        self.sleep(1)
        print("退出成功")
    ...
    def combat_custom(self):
        ...
        if self.uiconfig.get('cjsxj_role_skill', '0') == "8-1":
            ...
            self.cjsxj_combat.tick(self.boss)
