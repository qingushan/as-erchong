...
class AutoModTask(BaseTask):
    ...
    def check_use_level_more_award(self):
        # 检查当前轮次是否需要使用委托手册
        if -1 in self.level_more_award_boci or self.now_level_boci in self.level_more_award_boci:
            # 使用
    ...
    def go_to_level(self):
        ...
        print("开始前往夜航手册副本")
        self.go_to_lilian()
        self.click_until_ocr(x=805, y=99, rect=[1103,138,1261,612], pattern="前往")
        self.sleep(1)
    ...
    def quit_level(self):
        ...
        self.sleep(1)
        self.click_color_to_color(common_color,"退出委托-确定",common_color,"副本退出-再次进行",x=797,y=429,out_time=60)
        self.sleep(1)
        print("退出成功")
    ...
    def combat(self):
        ...
        while 1:
            ...
            # 扼守支持无尽轮次：每轮结束选择掉落加成后继续战斗
            if self.level_type == '扼守':
                res = self.find_my_color(common_color, "波次结束界面")
                if res:
                    print("波次结束界面,开始校验")
                    self.sleep(1)
                    if not self.is_text_re_in_ocr(rect=[222,500,1053,543],pattern="[撒离继续桃战]+"):
                        print("校验失败,不是波次结束界面")
                        continue
                    ...
                    if self.uiconfig['refresh_time_is_execute_mihan'] == 'on':
                        ...
                        if res:
                            self.click_color_to_color(common_color, "波次结束界面", common_color, "副本退出-再次进行", x=359, y=520, out_time=60)
                            self.sleep(3)
                            return True

                    if now_boci >= self.level_boci:
                        self.await_until_ocr(rect=[323,501,454,546],pattern="离")
                        self.click_color_to_color(common_color, "波次结束界面", common_color, "副本退出-再次进行", x=359, y=520, out_time=60)
                        self.sleep(3)
                        print("战斗完成")
                        return True

                    self.click_color_to_color(common_color, "波次结束界面", common_color, "副本内选择掉落加成", x=889, y=521)
                    self.sleep(1)
    ...
    def run(self):
        ...
        for item in self.mod_config_list:
            ...
            # 轮次仅对扼守生效；旧配置缺少 boci 时按单轮执行
            if self.level_type == '扼守':
                self.level_boci = max(int(item.get("boci", 1)), 1)
            else:
                self.level_boci = 1
