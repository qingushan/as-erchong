...
class CloudAutoRoleBreakthroughTask(CloudBaseTask):
    ...
    def go_to_level(self):
        ...
        self.go_to_lilian()
        self.sleep(1)
        self.click_until_ocr(x=1137, y=372, rect=[940, 630, 1230, 700], pattern="开始挑战")
        self.sleep(1)

        res = self.await_until_ocr(rect=[940, 630, 1230, 700], pattern="开始挑战", time_out=3)
        if not res:
            print("进入角色突破副本选择界面失败")
            return False
    ...
    def go_in_level(self):
        ...
        is_ready = False
        res = self.is_text_re_in_ocr(rect=[940, 630, 1230, 700], pattern="开始挑战")
        if res:
            is_ready = self.click_until_ocr(x=res[0].x, y=res[0].y, rect=[240, 160, 1040, 230], pattern="确认选择", time_out=15)

        res = self.is_text_re_in_ocr(rect=[747,647,985,706], pattern="再次进行")
        if res:
            is_ready = self.click_until_ocr(x=res[0].x, y=res[0].y, rect=[240, 160, 1040, 230], pattern="确认选择", time_out=15)
        ...
        res = self.click_until_color(cloud_common_color, "角色血条-绿色", x=795, y=516, out_time=60)
        if not res:
            print("进入角色突破副本失败")
            return False
    ...
    def combat(self):
        ...
        while 1:
            ...
            res = self.is_text_re_in_ocr(rect=[833,503,974,544], pattern="(继续|挑战)")
            if res:
                self.sleep(1)
                ...
                if self.uiconfig["refresh_time_is_execute_mihan"] == "on":
                    ...
                    if res:
                        self.click_color_to_color(cloud_common_color,"波次结束界面",cloud_common_color,"副本退出-再次进行",x=388,y=521,out_time=60)
                        self.sleep(1)
                        return True
                if now_boci >= self.level_boci:
                    self.click_until_ocr(x=388, y=521, rect=[747,647,985,706], pattern="再次进行")
                    self.sleep(1)
                    return True
                else:
                    self.click_until_ocr(x=888,y=521,rect=[487,489,803,546],pattern="确认选择")
                    self.sleep(1)
