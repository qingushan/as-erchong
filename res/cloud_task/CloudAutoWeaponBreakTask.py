...
class CloudAutoWeaponBreakTask(CloudBaseTask):
    # 云-武器突破
    ...
    def test(self):
        ...
        self.fly_spear(count=11)
        self.sleep(3)

        # 左走
        self.walk_to_a(1000,after_sleep=1)
        self.walk_to_a(1000,after_sleep=1)
        ...
        for i in range(2):
            for i in range(3):
                res = self.is_text_re_in_ocr(rect=self.cloud_interaction_text_rect["单行"],pattern="[暴露引信]+")
                if res:
                    break
                self.sleep(0.5)
            if res:
                break
            self.walk_to_w(300,after_sleep=1)

        self.click(786,354,after_sleep=2)
        self.click(1170,580,after_sleep=2)

        # 判断是否成功开启机关
        res = None
        for i in range(10):
            res = self.find_my_color(cloud_weapon_tupo,"成功开启机关")
            if res:
                break
            self.sleep(0.5)
        ...
        if self.uiconfig['wuqi_tupo_moling_run'] == 'on':
            # 魔灵遁逃
            # 等挑战结束
            res = self.await_until_color(cloud_weapon_tupo,"右上角绿色图标",time_out=30)
            if not res:
                print("等待击败大烟花超时")
                return False
            ...
            self.walk_to_w(1000*3,after_sleep=0.2)
            self.walk_to_d(1000*3.5,after_sleep=0.2)
            self.walk_to_s(1000*5,after_sleep=0.2)
            self.walk_to_d(1000*4.3,after_sleep=0.2)
            ...
            self.walk_to_d(1000*5,after_sleep=0.2)
            ...
            res = None
            for i in range(10):
                res = self.find_my_color(cloud_weapon_tupo,"撤离点图标")
                if res:
                    break
                self.sleep(0.1)
            
            map_type = 0    # 0:前方是撤离点  1：右边是撤离点
            if res:
                if self.center_x - 100 < res.x < self.center_x + 100:
                    # 前方
                    map_type = 0
                else:
                    map_type = 1
            ...
            if map_type == 0:
                self.walk_to_a(1000*1.5,after_sleep=0.5)
            elif map_type == 1:
                self.fly_spear(count=2)
                self.sleep(2)
                ...
                self.walk_to_d(1000,after_sleep=0.5)
            ...
            self.walk_to_a(500,after_sleep=0.5)
